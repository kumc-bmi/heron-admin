function mkDate(parts) {
    ...
    d.setMilliseconds(parts[6]);
    return d;
}

function draw(chart_elt, data, date_key) {
    var i;
    for (i = 0; i < data.length; i++) {
	data[i].date = mkDate(data[i][date_key]);
    }
...
dataLinesGroup = null;
    ...
    svg = chart_elt.select('svg').select('g');
    if (svg.empty()) {
	svg = chart_elt
	    .append('svg:svg')
	    .attr('width', w)
    ...
    */
    ...
    $('svg circle').tipsy({
        ...
        title: function() {
            var d = this.__data__;
	    var start = mkDate(d.start_date), end = d.end_date ? mkDate(d.end_date) : null;
	    var dur = end ? Math.round((end - start) / 1000) : '?';

	    var item_qty = $(d.request_xml).find('item').length;
	    var panel_qty = $(d.request_xml).find('panel').length;

            return (d.status + ': ' + dur + ' sec.<br />' +
		    d.user_id + ': ' + d.query_name + '<br />' +
		    panel_qty + ' panels ' +
		    item_qty + ' items<br />' +
		    d.result_type_description + ': ' + d.set_size + ' patients<br />' +
		    (d.request_xml.indexOf('<constrain_by_value>') >= 0 ?
		     '<b>constrain_by_value</b><br />' : '') +
		    (d.request_xml.indexOf('<constrain_by_date>') >= 0 ?
		     '<b>constrain_by_date</b><br />' : '') +
		    (d.request_xml.indexOf('<invert>1<') >= 0 ?
		     '<b>exclusion</b><br />' : '')
		   );
        }
    });
